...
function calculateEMI() {
  ...
  const rate = parseFloat(document.getElementById('emi-rate').value) / 1200;
...
function calculateSIP() {
  const monthlyInvestment = parseFloat(document.getElementById('sip-investment').value);
  const rate = parseFloat(document.getElementById('sip-rate').value) / 1200;
  const tenure = parseFloat(document.getElementById('sip-tenure').value) * 12;

  const maturity = monthlyInvestment * (Math.pow(1 + rate, tenure) - 1) / rate * (1 + rate);
  document.getElementById('sip-result').innerText = `Your SIP Maturity Amount is ₹${maturity.toFixed(2)}`;
}
...
function calculateGST() {
  ...
  document.getElementById('gst-result').innerText = `GST Amount is ₹${gst.toFixed(2)} and Total Amount is ₹${total.toFixed(2)}`;
}

document.addEventListener('DOMContentLoaded', () => {
  document.getElementById('emi-tab').classList.add('active');
